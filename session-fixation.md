# Security Advisory — Session Fixation (login does not regenerate session ID)

> **Identifier:** Pending CVE assignment / Internal ID **CC-2026-22**
...
A vulnerability was identified in **CloudClassroom-PHP-Project 1.0** (Vishal Mathur — `mathurvishal`), in the component **loginlinkadmin.php, loginlinkfaculty.php, loginlinkstudent.php**, that allows **an attacker (None (precondition: fix victim's cookie); user interaction: Required (victim login))** to exploit a **Session Fixation** flaw.

No login handler calls `session_regenerate_id()`. The ID present before login is promoted to authenticated session. An attacker who fixes the victim's PHPSESSID assumes the account after the victim logs in. Score up to 8.0 (AC:L) if cookie is trivially fixable.

Successful exploitation may result in **Complete account takeover (student, teacher or admin) after victim login (C:H/I:H).**
...
| Component: loginlinkadmin.php, loginlinkfaculty.php, loginlinkstudent.php | 1.0 | None | Affected |
...
The vulnerability occurs due to **Session Fixation** in the component **loginlinkadmin.php**.

No login handler calls `session_regenerate_id()`. The ID present before login is promoted to authenticated session. An attacker who fixes the victim's PHPSESSID assumes the account after the victim logs in. Score up to 8.0 (AC:L) if cookie is trivially fixable.

**Root cause (source code snippet):**

**`loginlinkfaculty.php:2,19-25`**

```php
session_start();
...
$_SESSION["fidx"]=$row["FID"];  // authenticates WITHOUT session_regenerate_id()
header('Location:welcomefaculty.php');
```

### Necessary Conditions

- Authentication: None (precondition: fix victim's cookie)
- User Interaction: Required (victim login)
- Attack Vector: Remote (network) — method POST
- Preconditions: Fix victim's PHPSESSID (via XSS — abundant here, subdomain, MITM on HTTP, or cookie without SameSite).
...
- Complete account takeover (student, teacher or admin) after victim login (C:H/I:H)
...
1. Attacker obtains/sets known PHPSESSID and fixes it in victim's browser.
2. Victim logs in normally; server does NOT issue new Set-Cookie (ID kept).
3. Attacker reuses same PHPSESSID and is authenticated as the victim.
...
```text
File(s): loginlinkadmin.php, loginlinkfaculty.php, loginlinkstudent.php
Parameter(s): PHPSESSID (cookie)
Method: POST · Authentication: None (precondition: fix victim's cookie)
```
...
```text
No Set-Cookie on login; fixed session accesses 'Welcome Faculty : Reeta Grower'.
```

### Result

Reproduced live in authorized lab (http://192.168.95.131:9292/) on 02/08/2026, in a non-destructive manner. Observed behavior confirms the Session Fixation flaw.
...
```bash
FIX="attackerfixed1234567890abcdef"
curl -s -i -b "PHPSESSID=$FIX" -X POST http://192.168.95.131:9292/loginlinkfaculty.php \
  --data "fid=x&pass=' OR '1'='1"   # response WITHOUT Set-Cookie (ID not regenerated)
curl -s -b "PHPSESSID=$FIX" http://192.168.95.131:9292/welcomefaculty.php | grep "Welcome Faculty"
```
...
2. Configure the prerequisite: Fix victim's PHPSESSID (via XSS — abundant here, subdomain, MITM on HTTP, or cookie without SameSite).
3. Access the component **loginlinkadmin.php** (parameter(s): PHPSESSID (cookie)).
4. Send the request/input described in sections 7 and 8.
5. Observe the vulnerable result: No Set-Cookie on login; fixed session accesses 'Welcome Faculty : Reeta Grower'.
...
- Cookie `HttpOnly`, `Secure`, `SameSite=Strict` (see item 24).
...
```text
grep -Ei "(union|select|extractvalue|concat|<script|onerror|onload|</textarea)" access.log | grep "loginlinkadmin.php"
```
