# Security Advisory — Stored XSS in updatedetailsfromfaculty.php

> **Identifier:** Pending CVE assignment / Internal ID **CC-2026-33**
...
A vulnerability was identified in **CloudClassroom-PHP-Project 1.0** (Vishal Mathur — `mathurvishal`), in the component **updatedetailsfromfaculty.php**, that allows **an attacker (None to inject (via item 00); user interaction: Required (victim opens page that renders the data))** to exploit a **Stored / Persistent Cross-Site Scripting** flaw.

Fields `fname`, `faname`, `addrs`, `gender`, `phno`, `city`, `pass` are persisted without sanitization and re-displayed in `value="..."` attribute without HTML-encoding. SELF-SERVICE screen of teacher (myfid). FName varchar(50): accommodates vector. Executes on the teacher themselves and on admins who review the record.
...
The vulnerability occurs due to **Stored / Persistent Cross-Site Scripting** in the component **updatedetailsfromfaculty.php**.
...
1. Send POST to `updatedetailsfromfaculty.php` recording breakout payload in field `fname`.
...
Parameter(s): fname, faname, addrs, gender, phno, city, pass
...
fname=<value>&faname=<value>&addrs=<value>&gender=<value>&phno=<value>&city=<value>&pass=<value>
```
...
![Web execution evidence — 33-updatedetailsfromfaculty-stored-xss](evidencia-web-33-updatedetailsfromfaculty-stored-xss.png)

**b) Vulnerable code line** (`updatedetailsfromfaculty.php`):

![Source code evidence — 33-updatedetailsfromfaculty-stored-xss](evidencia-codigo-33-updatedetailsfromfaculty-stored-xss.png)
...
python3 ../_lib/xss_poc.py "http://192.168.95.131:9292" "updatedetailsfromfaculty.php?..." \
  fname '"><svg onload=alert(1)>' --fields fname,faname,addrs,gender,phno,city,pass
```
...
3. Access the component **updatedetailsfromfaculty.php** (parameter(s): fname, faname, addrs, gender, phno, city, pass).
4. Send the request/input described in sections 7 and 8.
5. Observe the vulnerable result: payload reflected WITHOUT encoding: "><svg onload=alert(1)>
...
- Values persisted via `updatedetailsfromfaculty.php` containing `<script`, `onerror=`, `onload=`, `<svg` or `</textarea>`.
