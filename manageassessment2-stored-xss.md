# Security Advisory — Stored XSS in manageassessment2.php

> **Identifier:** Pending CVE assignment / Internal ID **CC-2026-13**
...
A vulnerability was identified in **CloudClassroom-PHP-Project 1.0** (Vishal Mathur — `mathurvishal`), in the component **manageassessment2.php**, that allows **an attacker (None to inject (via item 00); user interaction: Required (victim opens page that renders the data))** to exploit a **Stored / Persistent Cross-Site Scripting** flaw.

Fields `ExamName`, `Q1`, `Q2`, `Q3`, `Q4`, `Q5` are persisted without sanitization and re-displayed inside `<textarea>` without HTML-encoding. Q1..Q5 (varchar 10000) are also rendered to students in takeassessment2.php.
...
The vulnerability occurs due to **Stored / Persistent Cross-Site Scripting** in the component **manageassessment2.php**.
...
```php
<textarea name="ExamName">...<?php echo $row[...]; ?></textarea>
```
...
1. Send POST to `manageassessment2.php` recording breakout payload in field `ExamName`.
...
Parameter(s): ExamName, Q1, Q2, Q3, Q4, Q5
...
ExamName=<value>&Q1=<value>&Q2=<value>&Q3=<value>&Q4=<value>&Q5=<value>
```
...
![Web execution evidence — 13-manageassessment2-stored-xss](evidencia-web-13-manageassessment2-stored-xss.png)

**b) Vulnerable code line** (`manageassessment2.php`):

![Source code evidence — 13-manageassessment2-stored-xss](evidencia-codigo-13-manageassessment2-stored-xss.png)
...
python3 ../_lib/xss_poc.py "http://192.168.95.131:9292" "manageassessment2.php?..." \
  ExamName '</textarea><script>alert(document.domain)</script>' --fields ExamName,Q1,Q2,Q3,Q4,Q5 --textarea
```
...
3. Access the component **manageassessment2.php** (parameter(s): ExamName, Q1, Q2, Q3, Q4, Q5).
...
- Values persisted via `manageassessment2.php` containing `<script`, `onerror=`, `onload=`, `<svg` or `</textarea>`.
