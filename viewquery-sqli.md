# Security Advisory — SQL Injection in viewquery.php (parameter eid)

> **Identifier:** Pending CVE assignment / Internal ID **CC-2026-16**
...
A vulnerability was identified in **CloudClassroom-PHP-Project 1.0** (Vishal Mathur — `mathurvishal`), in the component **viewquery.php**, that allows **an attacker (None (via Broken Access Control — item 00; design would require student session); user interaction: None)** to exploit a **SQL Injection (UNION-based / error-based)** flaw.
...
| Component: viewquery.php | 1.0 | None | Affected |
...
The vulnerability occurs due to **SQL Injection (UNION-based / error-based)** in the component **viewquery.php**.
...
**b) Vulnerable code line** (`viewquery.php`):
...
3. Access the component **viewquery.php** (parameter(s): eid).
...
- Requests to `viewquery.php` with `UNION`, `SELECT`, `extractvalue`, `concat`, single quotes or `-- ` in parameter `eid`.
- Database error messages (e.g., `XPATH syntax error`, MariaDB/MySQL errors) reflected in responses.

### Example Log Search

```text
grep -Ei "(union|select|extractvalue|concat|<script|onerror|onload|</textarea)" access.log | grep "viewquery.php"
```
